function admin() {
  ...
  let users;
  fetch(`https://nameless-plateau-52452.herokuapp.com/show-admin/`)
...
function getAcc() {
  fetch(`https://nameless-plateau-52452.herokuapp.com/accounts/`)
    .then((response) => response.json())
    .then((json) => {
      console.log(json);
...
function deleteAcc(id) {
  if (confirm("are you sure you want to delete your account")) {
    fetch(
      `https://nameless-plateau-52452.herokuapp.com/delete-account/${id}/`,
      {
        method: "DELETE",
      }
    );
